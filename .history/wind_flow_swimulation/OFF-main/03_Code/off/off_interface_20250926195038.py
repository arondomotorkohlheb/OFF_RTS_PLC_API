...
class OFFInterface:
    ...
    def init_simulation_by_path(self, path_to_yaml: str):
        ...
        tmp_yaml_path = self._gen_FLORIS_yaml(settings_wke,
                                              sim_info["wind_farm"],
                                              sim_info["ambient"],
                                              os.path.dirname(path_to_yaml))  # changes this
    ...
    def _run_yaml_to_wind_farm(self, sim_info: dict) -> wfm.WindFarm:
        ...
        for idx in range(len(sim_info["wind_farm"]["farm"]["turbine_type"])):
            ...
            turbines.append(tur.TurbineMask(np.array([sim_info["wind_farm"]["farm"]["layout_x"][idx] * dist_factor,
                                            ...
                                            # Turb. states
                                            ops.FLORIDynOPs4(sim_info["solver"]["settings"]["n_op"]),  # OP model
                                            amb.FLORIDynAmbient(sim_info["solver"]["settings"]["n_op"]),  # Ambient model  # Turbine data


        wind_farm = wfm.WindFarm(turbines)
        return wind_farm

    def _run_yaml_to_wind_farm_reduced(self, sim_info: dict) -> wfm.WindFarm:
        pass
    ...
    def _gen_FLORIS_yaml(self, settings_wke: dict, settings_wf: dict, settings_amb: dict, path_to_tmp: str) -> str:
        ...
        stream = open(Path(off.OFF_PATH) / settings_wke["floris_logging"], 'r')
        f_logging = yaml.safe_load(stream)
        floris_file.update(f_logging)

        stream = open(Path(off.OFF_PATH) / settings_wke["floris_solver"], 'r')
        f_solver = yaml.safe_load(stream)
        floris_file.update(f_solver)

        stream = open(Path(off.OFF_PATH) / settings_wke["floris_wake"], 'r')
