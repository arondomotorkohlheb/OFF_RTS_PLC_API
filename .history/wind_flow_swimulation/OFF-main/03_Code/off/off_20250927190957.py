...
class OFF:
    ...
    def __init__(self, wind_farm: wfm.WindFarm, settings_sim: dict, settings_wke: dict, settings_sol: dict,
        ...
        if settings_ctr == {} or settings_ctr is None:
            ...
            lg.info("No controller specified, running open-loop simulation.")
        else:
    ...
    def run_one_step(self, t):
        """
        Runs one time step of the simulation. the information it needs from the RTS are:
        for each turbine:
            turbine states
        ...
        for idx, tur in enumerate(self.wind_farm.turbines):
            ...
            # power here is computed with turbine and wake 
            # pow_t[idx, :] = tur.calc_power(util.ot_uv2abs(uv_r[idx, 0], uv_r[idx, 1]), 1.225)
            ...
            if self.settings_vis["debug"]["effective_wf_tile"]:
        ...
        # ///////////////////// PROPAGATE /////////////////////
        for idx, tur in enumerate(self.wind_farm.turbines):
            # iterate state and keep only does a shift in the state array
            tur.ambient_states.iterate_states_and_keep()
            tur.turbine_states.iterate_states_and_keep() this is done on the RTS
