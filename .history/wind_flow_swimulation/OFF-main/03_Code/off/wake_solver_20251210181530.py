...
class TWFSolver(WakeSolver):
    ...
    def _get_wind_speeds_rp_reduced(self, wind_farm: wfm.WindFarm) -> tuple:
        ...
        u_eff_array = np.zeros(wind_farm.nT)
        v_eff_array = np.zeros(wind_farm.nT)
        uv_eff_array = np.zeros((wind_farm.nT, 2))
        for i_t in range(wind_farm.nT):
            [u_eff, v_eff] = ot.ot_abs2uv(ueff[i_t], ambient_states[i_t].get_turbine_wind_dir())
            u_eff_array[i_t] = u_eff
            v_eff_array[i_t] = v_eff
        
        return np.array([u_eff_array]), turbulance_intensity
