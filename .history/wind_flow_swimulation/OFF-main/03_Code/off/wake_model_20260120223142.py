...
class Floris4Wake(WakeModel):
    ...
    def set_wind_farm(self, wind_farm_layout: np.ndarray, turbine_states, ambient_states, turbulence_intensities):
        """
        Changes the states of the stored wind farm
        ...
        self.turbine_states = turbine_states
        self.ambient_states = ambient_states
C:\Users\kohlh\OneDrive - Delft University of Technology\Desktop\Study\MSc\research assignment\git\OFF_RTS_PLC_API\wind_flow_swimulation\OFF-main\03_Code\off\wake_model.py
        # retrieve yaw angles
        n_t = len(self.turbine_states)
    ...
    def get_measurements_reduced(self) -> tuple:
        """
        Returns the measurements of the wake model including the effective wind speed for all turbines
        ...
        -------
        tuple:
            np array((float, float)): u_eff for each turbine
            np array(float): turbulance_intensity for each turbine
        """
        
        # Solve the wind farm
        self.fmodel.run() # this only computes the flow field

        # Retrieve the measurements from fmodel (really this calls floris_model.core in all cases)
        avg_vel = self.fmodel.turbine_average_velocities
        turbulance_intensity = self.fmodel.get_turbine_TIs()

        return avg_vel.flatten(), turbulance_intensity.flatten()
