...
class TurbineMask(Turbine):
    ...
    def __init__(self, base_location: np.ndarray, orientation: np.ndarray, turbine_states: TurbineStates, observation_points: ObservationPoints, ambient_states: AmbientStates):
        ...
        self.ambient_states = ambient_states
        self.turbine_states = turbine_states
        lg.info("Turbine of type mask created")
...
class TurbineStatesFLORIDyn(TurbineStates):
    ...
    def get_ct(self, index: int) -> float: # this is never called in the original set up
        """
        get_ct(index) returns the Ct coefficient at a requested index of the turbine state chain
...
class TurbineStatesMask(TurbineStates):
    ...
    def set_all_states(self, states):
        ...
        ----------
        states : array-like
            State values in the order [cp, ct, yaw]
        """
