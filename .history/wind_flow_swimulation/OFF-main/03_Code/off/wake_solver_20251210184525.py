...
class TWFSolver(WakeSolver):
    ...
    def get_measurements_reduced(self, wind_farm: wfm.WindFarm) -> tuple:
        """
        Get the wind speed at the location of all OPs and the rotor plane of turbine with index i_t
        ...
        tuple(np.ndarray, np.ndarray)
            [u,v] wind speeds at the rotor plane (entry 1) and OPs (entry 2)
        """
        u_rp, turbulance_intensity = self._get_wind_speeds_rp_reduced(wind_farm)
        # u_op = self._get_wind_speeds_op(i_t, wind_farm)
        return u_rp, turbulance_intensity #, u_op
    ...
    def _get_wind_speeds_rp(self, i_t: int, wind_farm: wfm.WindFarm) -> tuple:
        ...
        # understanding: for each turbine, only consider the influencing turbines defined in dependencies
    ...
    def _get_wind_speeds_rp_reduced(self, wind_farm: wfm.WindFarm) -> tuple:
        """
        Calculates the effective wind speed at the rotor plane of turbine i_t
        Parameters
        ----------
        ...
        # loading data for the wake solver to solve


        wind_farm_layout = wind_farm.get_layout()
        turbine_states_lst = [] # turbine states
        ambient_states_lst = [] # ambient state # one for each observation point
        turbulance_intensities = np.zeros(wind_farm.nT) # turbulance intensity for each turbine

        # wind_farm.turbines[0].turbine_states = ['axial induction (-), yaw (deg), added turbulence intensity (%)']
        # wind_farm.turbines[0].ambient_states = ['Abs. wind speed (m/s)', 'Wind direction (deg)', 'Ambient turbulence intensity (%)']

        # go through each turbine to build the TWF
        for i_t in range(wind_farm.nT):
            turbine_states_lst.append(wind_farm.turbines[i_t].turbine_states.create_interpolated_state(0, 1, 0, 1))
            ambient_states_lst.append(wind_farm.turbines[i_t].ambient_states.create_interpolated_state(0, 1, 0, 1))
            print(ambient_states_lst[-1].get_all_states())
            exit()
            turbulance_intensities[i_t] = ambient_states_lst[-1].get_all_states()[2]


        print(turbulance_intensities)

        
        exit()

        # update wind farm properties
        self.floris_wake.set_wind_farm(wind_farm_layout, turbine_states, ambient_states, turbulance_intensities)

        ueff,  turbulance_intensities = self.floris_wake.get_measurements_reduced()
        lg.info('Effective wind speeds of all turbines: %s m/s' % (ueff))
        lg.info('Turbulance intensity of all turbines: %s m/s' % ( turbulance_intensities))

        uv_eff_array = np.zeros((wind_farm.nT, 2))
        for i_t in range(wind_farm.nT):
            [u_eff, v_eff] = ot.ot_abs2uv(ueff[i_t], ambient_states[i_t].get_turbine_wind_dir())
            uv_eff_array[i_t, :] = np.array([u_eff, v_eff])
        
        return uv_eff_array, turbulance_intensities
