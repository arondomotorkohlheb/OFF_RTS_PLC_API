...
from off.turbine import Turbine, TurbineStates
import numpy as np
...
class WindFarm:
    ...
    def get_current_turbine_state_from_external(self, turbine_state_list: List[TurbineStates]):
        ...
        Parameters
        ----------
        turbine_state_list: List[TurbineStates]
            List of turbine states to get the current states from
        ...
        np.ndarray:
            [n_t x m] matrix with current turbine states at the turbine locations
        """
        tmp_t_state = turbine_state_list[0].get_ind_state(0)
        t_states = np.zeros((len(turbine_state_list), len(tmp_t_state)))
        for idx, trb_state in enumerate(turbine_state_list):
            t_states[idx, :] = trb_state.get_ind_state(0)

        lg.info('Current wind farm states from external source:')
        lg.info(t_states)
