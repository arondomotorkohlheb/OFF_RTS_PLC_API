...
def main():
    ...
    simulink_input = np.array([20, 270/180*np.pi, 0.1]) # wind speed, wind direction (rad), TI
    ...
    for t in np.arange(off_interface.off_sim.settings_sim['time start'],
        ...
        while True:
            try:
                simulink_send_data = struct.pack('!ddd', simulink_input[0], simulink_input[2], simulink_input[21])
