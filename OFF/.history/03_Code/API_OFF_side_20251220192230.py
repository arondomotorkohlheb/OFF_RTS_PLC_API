...
def main():
    ...
    Nt = 2 # number of turbines, change to read from config file later
    ...
    file_name = f'{off.OFF_PATH}\\02_Examples_and_Cases\\03_Cases\\windfarm_information_2x3.yaml'
    ...
    simulink_input_init_perturbine = np.array([20, 0.06, -60/180*np.pi]) # wind speed, wind direction (rad), TI
    print(len(off_interface.off_sim.wind_farm.turbines))
    exit()
    simulink_input = np.array([simulink_input_init_perturbine for turbine in off_interface.off_sim.wind_farm.turbines])
    print('Initial simulink_input:', simulink_input)
    exit()

    off_input_init_per_turbine = np.array([0, 0, 0]) # initial yaw, cp, ct
    off_input = np.array([off_input_init_per_turbine for turbine in off_interface.off_sim.wind_farm.turbines])
    print('Initial off_input:', off_input)
    exit()
    ...
    for t in np.arange(off_interface.off_sim.settings_sim['time start'],
        ...
        while True:
            try:
                simulink_send_data = struct.pack('!dddddd', *simulink_input)
        ...
        print(simulink_input.shape)
        simulink_input[:, 1] = simulink_input[:, 1] * np.pi / 180  # convert to radians
        # for now only duplicate the data for one turbine
        simulink_input = np.concatenate((simulink_input, simulink_input), axis=0)
        ...
        off_input = np.array(struct.unpack('!dddddd', off_input_data))  # reevies array
