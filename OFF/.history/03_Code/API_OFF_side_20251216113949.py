...
def main():
    ...
    for t in np.arange(off_interface.off_sim.settings_sim['time start'],
        ...
        while True:
            try:
                simulink_send_data = struct.pack('!ddd', simulink_input[0], simulink_input[1], simulink_input[2])
        ...
        off_input = np.array(struct.unpack('!ddd', off_input_data))  # change
        off_input[:, 1] = off_input[:, 1] * 180 / np.pi  # convert to degrees
