...
def main():
    ...
    for t in np.arange(off_interface.off_sim.settings_sim['time start'],
        ...
        print(off_interface.off_sim.wind_farm.turbines[0].turbine_states.create_interpolated_state(0, 1, 0, 1))
        simulink_input = off_interface.off_sim.run_one_step(t)
        print(f"Time: {t}, Simulink input: {simulink_input}")
        
        # update turbine states 
        print(off_interface.off_sim.wind_farm.turbines[0].create_interpolated_state(0, 1, 0, 1))
        off_interface.off_sim.wind_farm.turbines[0].update_states(ct=0.8, cp=0.4, yaw=10.0)
        print(off_interface.off_sim.wind_farm.turbines[0].create_interpolated_state(0, 1, 0, 1))
        iteration += 1
        exit()
