...
class TWFSolver(WakeSolver):
    ...
    def get_measurements_reduced(self, i_t: int, wind_farm: wfm.WindFarm) -> tuple:
        ...
        tuple(np.ndarray, np.ndarray)
            [u,v] wind speeds at the rotor plane (entry 1) and OPs (entry 2)
        """
        ueff, wind_dir, turbulence_intensity = self._get_wind_speeds_rp_reduced(i_t, wind_farm)
        u_op = self._get_wind_speeds_op(i_t, wind_farm)
        return np.array([ueff, wind_dir, turbulence_intensity]), u_op
    ...
    def _get_wind_speeds_rp(self, i_t: int, wind_farm: wfm.WindFarm) -> tuple:
        ...
        for idx in np.arange(inf_turbines.shape[0]):
            if idx == i_t_tmp:
                ...
                twf_t_states.append(wind_farm.turbines[i_t_tmp].turbine_states.create_interpolated_state(0, 1, 1, 0))
                twf_a_states.append(wind_farm.turbines[i_t_tmp].ambient_states.create_interpolated_state(0, 1, 0, 0))
                continue
    ...
    def _get_wind_speeds_rp_reduced(self, i_t: int, wind_farm: wfm.WindFarm) -> tuple:
        ...
        # Get the measurements
        ueff, turbulence_intensity= self.floris_wake.get_measurements_i_t_reduced(i_t_tmp)
        lg.info('Effective wind speed of turbine %s : %s m/s' % (i_t, ueff))
        return ueff, twf_a_states[i_t_tmp].get_turbine_wind_dir(), turbulence_intensity
