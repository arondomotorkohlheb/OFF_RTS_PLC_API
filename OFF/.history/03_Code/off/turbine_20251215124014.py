...
class TurbineStatesMask(TurbineStates):
    ...
    def get_cp(self) -> float:
        """
        get_cp() returns the current Cp coefficient of the turbine
    ...
    def set_cp(self, cp: float):
        """
        Sets the current power coefficient of the turbine
    ...
    def set_ct(self, ct: float):
        """
        Sets the current thrust coefficient of the turbine
    ...
    def update_states(self, ct : float, cp :float, yaw : float):
        self.set_cp(cp)
        self.set_ct(ct)
        self.set_yaw(yaw)
        self.set_ax_ind((1 - np.sq))
...
class TurbineMask(Turbine):
    diameter = 160  # Meter
    nacellePos = np.array([0, 0, 120])  # in Meter
    turbine_type = "mask"

    def __init__(self, base_location: np.ndarray, orientation: np.ndarray, turbine_states: TurbineStatesMask,
                observation_points: ObservationPoints, ambient_states: AmbientStates, turbine_data: dict):
        """
        TurbineMask extends the base turbine and defines a shell that can be used to interface OFF with an external source
        ----------
        base_location : np.ndarray
            1 x 3 vector with x, y, z position of the turbine base (m)
        orientation : np.ndarray
        ...
        ambient_states : AmbientStates
            Ambient states object
        pased_down_states : cp and ct values from external source
        """
        self.diameter = turbine_data["rotor_diameter"]
        self.nacellePos = np.array([0, 0, turbine_data["hub_height"]])
        self.power_calc_method = "cp-u lut"  # alternative to "axial induction", "cp-bpa-tsr"  TODO: Set later in input
        ...
        super().__init__(base_location, orientation, turbine_states, observation_points, ambient_states)
        lg.info("Mask turbine of type " + turbine_data["name"] + "created")
        lg.info('Turbine base location: %s' % base_location)
        lg.info('Power calculation method: %s' % self.power_calc_method)
        lg.info('Thrust calculation method: %s' % self.thrust_calc_method)

    def calc_power(self, wind_speed, air_direction):
        return self.turbine_states.get_cp() * 0.5 * np.pi * (self.diameter / 2) ** 2 * wind_speed ** 3 * 1.225 \
                * np.cos(np.deg2rad(self.calc_yaw(air_direction)))**3
