...
class OFF:
    ...
    def __init__(self, wind_farm: wfm.WindFarm, settings_sim: dict, settings_wke: dict, settings_sol: dict,
        ...
        # =========== Controller ===========
        if settings_ctr == {} or settings_ctr is None:
            self.controller = None
            # lg.info("Controller is not defined in the OFF framework.")
        elif settings_ctr["ctl"] == "IdealGreedyBaseline":
    ...
    def __dir_init__(self, settings_sim: dict):
        ...
        try:
            ...
            if os.getcwd().endswith("03_Code"):
                root_dir = data_dir or f'{os.getcwd()[:-len("03_Code")]}\\runs\\'
            else:
                root_dir = data_dir or f'{os.getcwd()}\\runs\\'
    ...
    def run_one_step(self, t: float) -> tuple:
        """
        Function which executes one time step of the simulation and manipulates the ``self.wind_farm object``
        Parameters
        ------- 
        t : float
        ...
        -------
        tuple :
            Measurements from the current time step and control applied
        """ 
        lg.info('running simulation at time step: %s s.' % t)

        simulink_input = np.zeros((len(self.wind_farm.turbines), 3))
        ...
        for idx, tur in enumerate(self.wind_farm.turbines):
            ...
            simulink_input[idx, :], uv_op= self.wake_solver.get_measurements_reduced(idx, self.wind_farm)
        ...
        lg.info('simulink input for all turbines:')
        lg.info(simulink_input)
    ...
    def run_sim(self) -> tuple:
        ...
        for t in np.arange(self.settings_sim['time start'],
            ...
            for idx, tur in enumerate(self.wind_farm.turbines):
                ...
                pow_t[idx, :] = tur.calc_power(util.ot_uv2abs(uv_r[idx, 0], uv_r[idx, 1]), np.arctan2(uv_r[idx, 1], uv_r[idx, 0])*180/np.pi)
