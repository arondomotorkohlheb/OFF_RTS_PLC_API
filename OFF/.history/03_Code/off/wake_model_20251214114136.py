...
class Floris4Wake(WakeModel):
    ...
    def set_wind_farm(self, wind_farm_layout: np.ndarray, turbine_states, ambient_states):
        ...
        time_series = TimeSeries(
            ...
            turbulence_intensities=np.ones(1)*0.06, # TODO: Replace with actual value
        )
        ...
        for ii_t in range(n_t):
            windsirections.append(ambient_states[ii_t].get_turbine_wind_dir())
            windspeeds.append(ambient_states[ii_t].get_turbine_wind_speed_abs())
            turbulenceintensities.append(0.06)  # TODO: Replace with actual value
        
        time_series = TimeSeries(
            wind_directions=np.array(windsirections),
            wind_speeds=np.array(windspeeds),
            turbulence_intensities=np.array(turbulenceintensities),
        )
