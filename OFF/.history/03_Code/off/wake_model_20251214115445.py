...
class PythonGaussianWake(WakeModel):
    ...
    def get_measurements_i_t(self, i_t: int) -> tuple:
        ...
        print("python gaussian wake model called for turbine ")
        exit()
        ...
        ti      = np.array([s.get_current_ti()  for s in self.turbine_states])
...
class Floris4Wake(WakeModel):
    ...
    def set_wind_farm(self, wind_farm_layout: np.ndarray, turbine_states, ambient_states):
        ...
        windsirections = np.zeros(n_t)
        windspeeds = np.zeros(n_t)
        turbulenceintensities = np.zeros(n_t)
        for ii_t in range(n_t):
            windsirections[ii_t] = ambient_states[ii_t].get_turbine_wind_dir()
            windspeeds[ii_t] = ambient_states[ii_t].get_turbine_wind_speed_abs()
            turbulenceintensities[ii_t] = ambient_states[ii_t].get_turbine_turbulance_intensity()
        

        print(ambient_states[0].get_turbine_turbulance_intensity())
        exit()
        time_series = TimeSeries(
            wind_directions=windsirections,
            wind_speeds=windspeeds,
            turbulence_intensities=turbulenceintensities,
        )

        print(time_series.turbulence_intensities)
        print(time_series.wind_speeds)
        print(time_series.wind_directions)
        exit()
    ...
    def get_measurements_i_t(self, i_t: int) -> tuple:
        ...
        # Solve the wind farm
        print("old_get_measurements_i_t called")
        self.fmodel.run()
    ...
    def get_measurements_i_t_reduced(self, i_t: int) -> tuple:
        """
        Returns the reduced measurements of the wake model including the effective wind speed at the turbine i_t
        ...
        # Return the effective wind speed and the measurements
        return avg_vel.flatten()[i_t], TIs.flatten()[i_t]
