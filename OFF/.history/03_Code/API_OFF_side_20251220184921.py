...
def main():
    ...
    for t in np.arange(off_interface.off_sim.settings_sim['time start'],
        ...
        while True:
            try:
                simulink_send_data = struct.pack('!dddddd', simulink_input[0], simulink_input[2], simulink_input[1])
        ...
        off_input = np.array(struct.unpack('!dddddd', off_input_data))  # convert to array
        # convert to a matrix with shape (num_turbines, 3)
        off_input = off_input.reshape((-1, 3))
        print(off_input)
        print(off_input.shape)
        exit()
