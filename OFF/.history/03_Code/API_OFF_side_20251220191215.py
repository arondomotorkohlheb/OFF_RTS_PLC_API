...
def main():
    ...
    simulink_input = np.array([20, 0.06, -60/180*np.pi]) # wind speed, wind direction (rad), TI
    simulink_input = np.concatenate((simulink_input, simulink_input), axis=0)  # for two turbines
    ...
    for t in np.arange(off_interface.off_sim.settings_sim['time start'],
        ...
        for turbine_index, turbine in enumurate(off_interface.off_sim.wind_farm.turbines):
            turbine.turbine_states.update_states(ct=off_input[iteration, 2], cp=off_input[iteration, 1], yaw=off_input[iteration, 0])
        iteration += 1
