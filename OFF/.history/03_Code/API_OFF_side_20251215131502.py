...
def main():
    ...
    for t in np.arange(off_interface.off_sim.settings_sim['time start'],
        ...
        print(off_interface.off_sim.wind_farm.turbines[0].turbine_states.create_interpolated_state(0, 1, 1, 0).get_all_states())
        off_interface.off_sim.wind_farm.turbines[0].turbine_states.update_states(ct=0.8, cp=0.4, yaw=10.0)
        print(off_interface.off_sim.wind_farm.turbines[0].turbine_states.create_interpolated_state(0, 0, 1, 0).get_all_states())
        iteration += 1
        exit()
