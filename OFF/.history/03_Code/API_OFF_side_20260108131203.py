...
def main():
    ...
    Nt = len(off_interface.off_sim.wind_farm.turbines) # number of turbines
    ...
    for t in np.arange(off_interface.off_sim.settings_sim['time start'],
        ...
        while True:
            try:
                simulink_send_data = struct.pack('!' + 'd'*3*Nt, *simulink_input.T.flatten()) # sending array of string 
        ...
        simulink_input[:, 2] = simulink_input[:, 2] * np.pi / 180  # convert to radians        
        # print("sent to simulink:", simulink_input)

        for i in range(Nt):
            lg.info(f"Turbine {i+1} - sending to Simulink wind speed: {simulink_input[i,0]}, TI: {simulink_input[i,1]}, wind direction (rad): {simulink_input[i,2]}")
        ...
        if mode == 'Prod':
            ...
            simulink_output = np.array(struct.unpack('!' + 'd'*12*Nt, off_input_data))  # recieves flat array
            # reshape to (num_turbines, 12)
            simulink_output = simulink_output.reshape((12, -1)).T
            off_input = simulink_output[:, [9, 10, 11]]  # extract yaw, cp, ct
            visual_data.append(simulink_output[:, :9])  # extract visual data if needed
        else:
            raise ValueError("Mode not recognized. Choose 'Visual' or 'Prod'.")
    ...
    # make graphs for all 10 columns in visual data
    if mode == 'Visual':
        

        visual_data = np.array(visual_data)  # shape (time_steps, num_turbines, 9)
        time_steps = visual_data.shape[0]
        time_array = np.arange(0, time_steps * off_interface.off_sim.settings_sim['time step'], off_interface.off_sim.settings_sim['time step'])

        for col in range(9):
            plt.figure()
            for turbine_index in range(Nt):
                plt.plot(time_array, visual_data[:, turbine_index, col], label=f'Turbine {turbine_index+1}')
            plt.xlabel('Time (s)')
            plt.ylabel(f'Visual Data Column {col+1}')
            plt.title(f'Visual Data Column {col+1} over Time')
            plt.legend()
            plt.grid()
            plt.show()
