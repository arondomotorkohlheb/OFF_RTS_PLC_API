...
def main():
    ...
    simulink_input = np.array([20, 120/180*np.pi, 0.01]) # wind speed, wind direction (rad), TI
    ...
    for t in np.arange(off_interface.off_sim.settings_sim['time start'],
        ...
        logging.info(f"Iteration {iteration}: received from Simulink yaw: {off_input[0]}, cp: {off_input[1]}, ct: {off_input[2]}")
