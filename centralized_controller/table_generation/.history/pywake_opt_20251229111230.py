...
from py_wake.wind_turbines.power_ct_functions import PowerCtTabular, YawPowerCtFunction
...
if __name__ == "__main__": 
    ...
    yaw_power_ct = YawPowerCtFunction(
    powerCtFunction=base_power_ct,
    p_power=3,
    p_ct=3)
    ...
    windTurbines = WindTurbines(
        ...
        powerCtFunctions=[yaw_power_ct]
    )

    ws = np.array([0, 4, 6, 8, 10, 12, 15, 25])
    power = np.array([0, 0, 300e3, 900e3, 1600e3, 2000e3, 2000e3, 0])
    ct = np.array([0, 0.8, 0.8, 0.8, 0.75, 0.7, 0.6, 0])

    power_ct = PowerCtTabular(ws, power, ct)
