...
if __name__ == "__main__": 
    ...
    best_power = simulationResult_opt.Power.sum().values
    power_yaw_dict[yaw] = best_power
    best_yaw = 0

    
    while relative_power_improvement > 0.01:
        previous_best_power = best_power
        for yaw_i in [left_end_point, right_end_point]:
            ...
            power_yaw_dict[yaw] = sumpower
            sumpower = simulationResult_opt.Power.sum().values
        ...
        new_range = right_end_point - left_end_point
        left_end_point = best_yaw_i - (right_end_point - left_end_point)/4
            
        yaw[opt_order[0]] = best_yaw

    print(f'Best yaw for turbine {opt_order[0]} is {best_yaw} degrees with total power {best_power} MW')
