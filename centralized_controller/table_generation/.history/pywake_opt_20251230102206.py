...
if __name__ == "__main__": 
    ...
    for i in range(len(layout_x)):
        _, wind_aligned_distance[i] = rotate_points(layout_x[i], layout_y[i], np.deg2rad(wd))

    sort_idx = np.argsort(wind_aligned_distance)
    print('Turbine order based on wind direction:')
    print(sort_idx + 1)  # +1 to convert from 0-based to 1-based indexing
