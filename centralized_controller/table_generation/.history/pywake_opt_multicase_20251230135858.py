...
def optimize_yaw_angles(ws, wd, set_up_constants, ti = 0.01, n_points_per_range = 6, search_radius = 25, improvement_margin = 0.01/100):
    ...
    for tur_ind in opt_order:
        ...
        while relative_power_improvement > improvement_margin:
            ...
            relative_power_improvement = (best_power - previous_best_power)/(previous_best_power+0.1)
...
if __name__ == "__main__":
    ...
    n_wind_directions = 300
    n_wind_speeds = 10
    n_turbines = set_up_constants[3]["n_turbines"]
    opt_yaw_wind_dir_speed = np.zeros((n_wind_directions, n_wind_speeds, n_turbines))
    wind_speeds = np.linspace(4, 30, n_wind_speeds)
    wind_directions = np.linspace(0, 360, n_wind_directions, endpoint=False)

    power_heat_map = np.zeros((n_wind_directions, n_wind_speeds))

    for i, wd in enumerate(tqdm(wind_directions, desc="wind directions", leave=False, ascii=True)):
        for j, ws in enumerate(wind_speeds):
            power, yaw = optimize_yaw_angles(ws=ws, wd=wd, set_up_constants=set_up_constants)
            opt_yaw_wind_dir_speed[i, j, :] = yaw
            power_heat_map[i, j] = power

            

    plt.imshow(power_heat_map, extent=[min(wind_speeds), max(wind_speeds), 0, 360], aspect='auto', origin='lower')
    plt.colorbar(label='Total Power (MW)')
    plt.xlabel('Wind Speed (m/s)')
    plt.ylabel('Wind Direction (degrees)')
    plt.title('Total Power Heatmap')
    plt.show()

    # Save the optimized yaw angles array to a .npy file
    np.save('optimized_yaw_angles2.npy', opt_yaw_wind_dir_speed)